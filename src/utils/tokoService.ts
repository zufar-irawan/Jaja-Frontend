import api from './api'
import { Product, SearchProductsParams, SearchProductsResponse } from './productService'
...
export async function getTokoProducts(
    ...
    try {
        ...
        console.error('Error fetching toko products:', error)
        return {
            success: false,
            code: 500,
...
export function isTokoOpen(dataString: string): boolean {
    ...
    return currentTime >= bukaTokoData.time_open && currentTime <= bukaTokoData.time_close
}

/**
 * Get store statistics (you might want to add API endpoint for this)
 */
...
export async function getTokoStats(slug: string): Promise<TokoStats> {
    try {
        ...
        return {
            ...
            totalSold: 5000, // Placeholder
            averageRating: 4.8, // Placeholder
            totalReviews: 2500 // Placeholder
        }
    } catch (error) {
