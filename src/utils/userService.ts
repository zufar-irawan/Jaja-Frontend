...
export async function editRekening(
  id: number,
  data: AddRekeningData,
): Promise<ApiResponse<any>> {
  try {
...
export async function getListRekening(): Promise<ApiResponse<Rekening[]>> {
  try {
    ...
    return {
      success: false,
      message:
        error.response?.data?.message || "Gagal mengambil daftar rekening",
    };
  }
}
...
export async function setPrimaryRekening(
  id: number,
): Promise<ApiResponse<void>> {
  try {
...
export async function getVillages(
  ...
  try {
    ...
    return {
      success: false,
      message:
        error.response?.data?.message || "Gagal mengambil data kelurahan",
    };
  }
}
