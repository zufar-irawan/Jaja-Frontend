import { deleteRekening, setPrimaryRekening, type Rekening } from "@/utils/userService"
...
export default function AccountListCard({ rekening, onEdit, onDeleted, onPrimaryChanged }: AccountListCardProps) {
    const [deleting, setDeleting] = useState(false)
    const [settingPrimary, setSettingPrimary] = useState(false)

    const cardClassName = useMemo(() => {
        const base = "group overflow-hidden rounded-xl border bg-white shadow-sm transition-all hover:shadow-md"
    ...
    const handleDelete = async () => {
        ...
        if (response.success) {
            await Swal.fire("Berhasil", response.message || "Rekening berhasil dihapus", "success")
            onDeleted?.()
        } else {
            await Swal.fire("Gagal", response.message || "Rekening gagal dihapus", "error")
        }
    }
    ...
    const handleSetPrimary = async () => {
        ...
        if (response.success) {
            await Swal.fire("Berhasil", response.message || "Rekening utama diperbarui", "success")
            onPrimaryChanged?.()
        } else {
            await Swal.fire("Gagal", response.message || "Rekening utama gagal diubah", "error")
        }
    }
    ...
    return (
        <div className={cardClassName}>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xl font-bold text-gray-800">{rekening.bank_name}</p>
                            <p className="text-xs text-gray-500">{rekening.alias_name}</p>
                        </div>
                        {rekening.is_primary && (
                            <span className="ml-2 rounded-full bg-blue-600/10 px-3 py-1 text-xs font-semibold text-blue-700">
                                Rekening Utama
                            </span>
                        )}
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                        <button
                            className={`rounded-lg border px-4 py-2 text-sm font-medium transition-colors ${rekening.is_primary ? 'border-blue-200 bg-blue-50 text-blue-700' : 'border-blue-200 bg-white text-blue-600 hover:bg-blue-50'}`}
                            onClick={handleSetPrimary}
                            disabled={settingPrimary || rekening.is_primary}
                        >
                            {rekening.is_primary ? 'Rekening Utama' : (settingPrimary ? 'Mengatur...' : 'Jadikan Utama')}
                        </button>
            ...
            <div className="space-y-4 p-6">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                    <div className="rounded-lg border border-gray-200 bg-white p-4">
                        ...
                        <p className="text-sm font-medium text-gray-800">{rekening.branch_office}</p>
                    </div>
                    <div className="rounded-lg border border-gray-200 bg-white p-4">
                        <p className="mb-1 text-xs font-medium uppercase tracking-wide text-gray-500">
                            Kota
            ...
            <div className="border-t border-gray-100 bg-gray-50/50 px-6 py-3">
                <p className={`flex items-center gap-2 text-xs ${rekening.verified ? 'text-green-600' : 'text-gray-500'}`}>
