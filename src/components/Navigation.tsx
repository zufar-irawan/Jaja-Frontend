...
export default function JajaNavbar() {
  ...
  const getUserInitials = () => {
    ...
    if (userProfile.nama_lengkap) return userProfile.nama_lengkap.charAt(0).toUpperCase();
                              ...
                              return (
                                <div className="py-4 px-3 max-h-[480px] overflow-y-auto">
                                  <div className="mb-4 px-2">
                                    <h3 className="font-bold text-gray-900 text-base">{activeCategory.kategori}</h3>
                ...
                <div className="absolute top-full mt-2 w-full bg-white rounded-2xl shadow-2xl border border-gray-100 max-h-[600px] overflow-y-auto z-60 animate-in fade-in slide-in-from-top-2 duration-200">
                  {isSearchLoading ? (
                    ...
                    <div className="py-12 text-center">
                      ...
                      <p className="text-gray-400 text-sm mt-1">Coba kata kunci lain atau periksa ejaan</p>
                    </div>
                  ) : (
                    <div className="py-3">
                      {/* Products Section */}
                      {searchResults.products.length > 0 && (
                        <div className="mb-4">
                          <div className="flex items-center gap-2 px-5 py-2 bg-gray-50">
                            ...
                            <h3 className="font-bold text-gray-700 text-sm">Produk</h3>
                          ...
                          <div className="divide-y divide-gray-100">
                            {searchResults.products.map((product: Product) => (
                              ...
                              >
                                <img
                                  src={product.covers?.[0]?.foto || "/api/placeholder/60/60"}
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className="font-medium text-gray-800 text-sm line-clamp-1 group-hover:text-[#55B4E5] transition-colors">
                                    {highlightText(product.nama_produk, searchQuery)}
                                  </p>
                                  <div className="flex items-center gap-2 mt-1">
                                    <span className="font-bold text-[#55B4E5] text-sm">{formatPrice(product.harga)}</span>
                                    {product.diskon > 0 && (
                                  ...
                                  <p className="text-xs text-gray-500 mt-0.5">{product.tokos?.nama_toko}</p>
                                </div>
                                {product.avg_rating != null && (
                                  <div className="flex items-center gap-1 text-xs">
                                    <span className="text-yellow-500">★</span>
                                    <span className="font-semibold text-gray-700">{Number(product.avg_rating).toFixed(1)}</span>
                                  </div>
                                )}
                              </a>
                            ))}
                      ...
                      {searchResults.stores.length > 0 && (
                        <div className="mb-4">
                          <div className="flex items-center gap-2 px-5 py-2 bg-gray-50">
                            ...
                            <h3 className="font-bold text-gray-700 text-sm">Toko</h3>
                            <span className="text-xs text-gray-500">({searchResults.stores.length})</span>
                          </div>
                          <div className="divide-y divide-gray-100">
                            {searchResults.stores.map((store: Product["tokos"]) => (
                              <a
                                key={store.id_toko}
                              ...
                              >
                                ...
                                <div className="flex-1">
                                  <p className="font-semibold text-gray-800 text-sm group-hover:text-[#FBB338] transition-colors">
                                    {highlightText(store.nama_toko, searchQuery)}
                                  </p>
                                  {store.wilayah && (
                                    <p className="text-xs text-gray-500 mt-0.5">{store.wilayah.kelurahan_desa}</p>
                                  )}
                      ...
                      {searchResults.categories.length > 0 && (
                        <div>
                          <div className="flex items-center gap-2 px-5 py-2 bg-gray-50">
                            ...
                            <h3 className="font-bold text-gray-700 text-sm">Kategori</h3>
                            <span className="text-xs text-gray-500">({searchResults.categories.length})</span>
                          </div>
                          <div className="divide-y divide-gray-100">
                            {searchResults.categories.map((category: SearchCategory) => (
                              <a
                                key={category.id_kategori}
                              ...
                              >
                                ...
                                <p className="font-medium text-gray-800 text-sm group-hover:text-purple-600 transition-colors">
                                  {highlightText(category.kategori, searchQuery)}
                                </p>
                              </a>
                            ))}
                          </div>
