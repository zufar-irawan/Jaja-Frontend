...
import { deleteAddress } from "@/utils/userService"
...
export default function AddressListCard({ alamat, onEdit, onOpen, onSelect, onDeleted }: AddressListCardProps) {
    ...
    const handleDelete = async () => {
        ...
        if (response.success) {
            await Swal.fire("Berhasil", response.message || "Alamat berhasil dihapus", "success")
            if (onDeleted) await onDeleted()
        } else {
            await Swal.fire("Gagal", response.message || "Alamat gagal dihapus", "error")
        }
    }
