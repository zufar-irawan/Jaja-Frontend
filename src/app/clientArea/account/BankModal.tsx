...
export default function BankModal({
  ...
  useEffect(() => {
    if (isEdit && rekening) {
      ...
      const exists = bankOptions.some(
        (bank) => bank.code === rekening.bank_code,
      );
  ...
  const handleInputChange =
    (field: keyof AddRekeningData) =>
    (event: ChangeEvent<HTMLInputElement>) => {
      let value = event.target.value;

      // Sanitize account number - only allow digits
      if (field === "account") {
        value = value.replace(/\D/g, "");
      }

      // Sanitize bank code for custom banks - only allow alphanumeric
      if (field === "bank_code" && isCustomBank) {
  ...
  const validateForm = () => {
    ...
    if (!form.bank_name.trim() || !form.bank_code.trim())
      return "Silakan pilih atau isi nama bank dan kode bank";
    if (form.bank_code.trim().length < 2) return "Kode bank minimal 2 karakter";

    if (!form.account.trim()) return "Nomor rekening wajib diisi";
    if (form.account.trim().length < 5) return "Nomor rekening minimal 5 digit";
    if (!/^\d+$/.test(form.account.trim()))
      return "Nomor rekening hanya boleh berisi angka";

    if (!form.alias_name.trim()) return "Nama alias rekening wajib diisi";
    if (form.alias_name.trim().length < 3)
      return "Nama alias minimal 3 karakter";

    if (!form.branch_office.trim()) return "Kantor cabang wajib diisi";
    if (form.branch_office.trim().length < 3)
      return "Kantor cabang minimal 3 karakter";

    if (!form.city.trim()) return "Kota/kabupaten wajib diisi";
    if (form.city.trim().length < 3) return "Kota/kabupaten minimal 3 karakter";
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const result =
        isEdit && rekening?.id_data
          ? await editRekening(rekening.id_data, payload)
          : await addRekening(payload);

      if (!result.success) {
        const errorMsg = result.message || "Gagal menyimpan rekening";
        setError(errorMsg);
        return;
      }
      ...
      console.error("Failed to save rekening", err);
      const errorMessage =
        err instanceof Error
          ? err.message
          : "Terjadi kesalahan tidak terduga saat menyimpan rekening";
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain">
          ...
          >
            ...
            <div className="space-y-1.5 sm:space-y-2">
              <label
                htmlFor="bankName"
                className="text-xs sm:text-sm font-medium text-gray-700 flex items-center"
              >
                <svg
            ...
            {isCustomBank && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1.5 sm:space-y-2">
                  ...
                  >
                    Nama Bank Custom <span className="text-red-500">*</span>
                  </label>
                  <input
                    id="customBankName"
                    type="text"
                ...
                <div className="space-y-1.5 sm:space-y-2">
                  <label
                    ...
                    className="text-xs sm:text-sm font-medium text-gray-700"
                  >
                    Kode Bank Custom <span className="text-red-500">*</span>
                  </label>
                  <input
                    id="customBankCode"
                    type="text"
