...
const ShoppingCartPage = () => {
      ...
      setCartItems((items) =>
        items.map((item) =>
          item.id_cart === id_cart
            ? { ...item, status_pilih: !item.status_pilih }
            : item,
        ),
      );
  ...
  const handleRemoveItem = async (id_cart: number) => {
    try {
      ...
      if (response.success) {
        setCartItems((items) =>
          items.filter((item) => item.id_cart !== id_cart),
        );
  ...
  const handleProceedToCheckout = async () => {
    try {
      ...
      const selectedItems = cartItems.filter((item) => item.status_pilih);
        ...
        await Swal.fire({
          ...
          html: `
            <div style="text-align: center;">
              ...
              <p style="color: #666;">
                Silakan pilih produk terlebih dahulu untuk melanjutkan checkout.
              </p>
            </div>
          `,
          icon: "warning",
      ...
      // Validasi: Cek apakah semua produk dari toko yang sama
      const storeIds = new Set(selectedItems.map((item) => item.id_toko));

      if (storeIds.size > 1) {
        // Ada lebih dari 1 toko
        const storeNames = Array.from(
          new Set(
            selectedItems.map(
              (item) => item.toko?.nama_toko || "Toko Tidak Diketahui",
            ),
          ),
        );

        await Swal.fire({
          title: "Tidak Bisa Checkout",
          html: `
              <p style="color: #666; margin-bottom: 8px;">
                Anda memilih produk dari <strong>${storeIds.size} toko berbeda</strong>.
              </p>
              <p style="color: #666; margin-bottom: 12px;">
                Silakan pilih produk dari <strong>satu toko saja</strong> untuk melanjutkan checkout.
              </p>
              </div>
            </div>
          `,
  ...
  const totals = calculateCartTotals(cartItems.map(toCartItem), 0, discount, 0);
  const allSelected =
    cartItems.length > 0 && cartItems.every((item) => item.status_pilih);
    ...
    return (
      ...
      >
        <div style={{ textAlign: "center", maxWidth: "400px" }}>
          <ShoppingCart
            size={64}
            style={{ margin: "0 auto 16px", color: "#dc3545", opacity: 0.5 }}
          />
                ...
                {Object.entries(groupedByStore).map(([storeId, storeData]) => (
                  <div key={storeId} style={{ marginBottom: "24px" }}>
                    ...
                    {storeData.items.map((item) => {
                      ...
                      const product =
                        typeof item.produk === "object" && item.produk
                          ? item.produk
                          : null;
                      const stock =
                        item.variasi?.stok_variasi || product?.stok || 99;
                      ...
                      return (
                        <div
                          ...
                          style={{
                            ...
                            borderRadius: "8px",
                            marginBottom: "12px",
                            backgroundColor: item.status_pilih
                              ? "#f0f8ff"
                              : "white",
                        ...
                        >
                          ...
                          <div
                            ...
                            onClick={() =>
                              handleNavigateToProduct(product?.slug_produk)
                            }
                            style={{
                              width: "80px",
                              height: "80px",
                            ...
                            >
                              ...
                              >
                                ...
                                <input
                                  type="number"
                                  value={
                                    editingQuantities[item.id_cart] ?? item.qty
                                  }
                                  onFocus={() => {
                                    setEditingQuantities((prev) => ({
                                      ...prev,
                                      [item.id_cart]: String(item.qty),
                                    }));
                                  }}
                                  onChange={(e) => {
                                    setEditingQuantities((prev) => ({
                                      ...prev,
                                      [item.id_cart]: e.target.value,
                                    }));
                                  }}
                                  onBlur={() => {
                                    const editedValue =
                                      editingQuantities[item.id_cart];
                                ...
                                <button
                                  ...
                                  style={{
                                    ...
                                    justifyContent: "center",
                                    opacity:
                                      item.qty >= stock || isUpdating ? 0.5 : 1,
                                    flexShrink: 0,
                                  }}
                                >
                                  <Plus size={14} />
            ...
            {totals.selectedCount === 0 ? (
              ...
              <>
                ...
                <button
                  ...
                  onMouseOver={(e) =>
                    (e.currentTarget.style.backgroundColor = "#4a9fcf")
                  }
                  onMouseOut={(e) =>
