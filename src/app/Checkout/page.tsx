...
import { Check, X, Tag, ChevronRight, MapPin, Truck, Package, Loader2, AlertCircle } from 'lucide-react';
import { useRouter } from 'next/navigation';
import { reviewCheckout, processCheckout, processPayment } from '@/utils/checkoutActions';
import { getCart } from '@/utils/cartActions';
import { formatCurrency, validateCheckoutData, groupItemsByStore, calculateItemTotal, type ShippingOption } from '@/utils/checkoutService';
...
const CheckoutPage = () => {
  ...
  const handleShippingSelect = (storeId: number, courier: string, service: string, cost: number) => {
  ...
  const calculateTotals = () => {
    ...
    const shippingCost = Object.values(selectedShipping).reduce((sum, shipping) => {
    ...
    return (
      <div style={{ 
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ marginTop: '16px', color: '#6c757d' }}>Memuat checkout...</p>
        </div>
      </div>
    );
  }
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ 
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '300px', overflowY: 'auto' }}>
                  {(Object.values(groupItemsByStore(selectedItems)) as { id_toko: number; nama_toko: string; items: CartItem[] }[]).map(store => (
                    <div key={store.id_toko} style={{ marginBottom: '12px' }}>
                      <div style={{ fontSize: '11px', fontWeight: '600', color: '#6c757d', marginBottom: '8px', paddingLeft: '8px' }}>
                        🏪 {store.nama_toko}
                      </div>
                      {store.items.map((item: CartItem) => (
                        <div key={item.id_cart} style={{
                          ...
                          <div style={{ 
                            ...
                            <img 
                              src={(typeof item.produk === 'object' && item.produk?.foto_produk) || item.produk_cover || 'https://via.placeholder.com/45'}
                              alt={(typeof item.produk === 'object' && item.produk?.nama_produk) || ''}
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <h3 style={{ 
                              ...
                              {(typeof item.produk === 'object' && item.produk?.nama_produk) || ''}
                            </h3>
                            <div style={{ fontSize: '9px', color: '#6c757d' }}>
                              {item.model_variasi && `${item.model_variasi} • `}Qty: {item.qty}
                            </div>
                          </div>
              ...
              <div style={{ 
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  <input
                    ...
                    onChange={(e) => setAddressForm(prev => ({ ...prev, nama_penerima: e.target.value }))}
                    style={{ 
                      width: '100%', 
                      padding: '10px', 
                  ...
                  <input
                    ...
                    value={addressForm.telp_penerima}
                    onChange={(e) => setAddressForm(prev => ({ ...prev, telp_penerima: e.target.value }))}
                    style={{ 
                      width: '100%', 
                      padding: '10px', 
                      border: '1px solid #dee2e6', 
                  ...
                  <textarea
                    ...
                    onChange={(e) => setAddressForm(prev => ({ ...prev, alamat_lengkap: e.target.value }))}
                    style={{ 
                      width: '100%', 
                      padding: '10px', 
                      border: '1px solid #dee2e6', 
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    ...
                    {['jne', 'jnt', 'sicepat'].map(courier => (
                      <label 
                        ...
                        style={{ 
                          ...
                          padding: '10px',
                          border: selectedShipping[store.id_toko]?.courier === courier ? '2px solid #55B4E5' : '1px solid #dee2e6',
                          borderRadius: '6px',
                          cursor: 'pointer',
                          backgroundColor: selectedShipping[store.id_toko]?.courier === courier ? '#f0f9ff' : 'transparent'
                        }}
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <input
                            ...
                            checked={selectedShipping[store.id_toko]?.courier === courier}
                            onChange={() => handleShippingSelect(store.id_toko, courier, 'REG', 15000)}
                          ...
                          <div>
                            ...
                            <div style={{ fontSize: '9px', color: '#6c757d' }}>REG - 2-3 hari</div>
                          </div>
                        </div>
              ...
              <div style={{ 
                ...
                <div style={{ 
                  ...
                  <h2 style={{ fontSize: '14px', fontWeight: '600', color: '#333' }}>Voucher</h2>
                </div>
                ...
                {appliedVoucher ? (
                  <div style={{ 
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'white' }}>
                      <Check size={14} />
                      <span style={{ fontSize: '11px', fontWeight: '600' }}>{appliedVoucher.code}</span>
                    </div>
                    <button
            ...
            <div style={{ 
              ...
              <div style={{ 
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: '11px', color: '#6c757d' }}>Subtotal</span>
                  <span style={{ fontSize: '11px', fontWeight: '600' }}>{formatCurrency(totals.subtotal)}</span>
                </div>
                
                {totals.discount > 0 && (
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: '11px', color: '#6c757d' }}>Diskon</span>
                    <span style={{ fontSize: '11px', fontWeight: '600', color: '#FBB338' }}>-{formatCurrency(totals.discount)}</span>
                  </div>
                )}
                
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: '11px', color: '#6c757d' }}>Ongkir</span>
                  <span style={{ fontSize: '11px', fontWeight: '600' }}>{formatCurrency(totals.shippingCost)}</span>
                </div>
                
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <span style={{ fontSize: '11px', color: '#6c757d' }}>Pajak (10%)</span>
                  <span style={{ fontSize: '11px', fontWeight: '600' }}>{formatCurrency(totals.tax)}</span>
                </div>
              </div>
              ...
              <div style={{ 
                ...
                <span style={{ fontSize: '20px', fontWeight: '700', color: '#FBB338' }}>
